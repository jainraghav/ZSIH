...
def train(model_s,model_i,optimizer,epoch,train_loader,logger):
    ...
    for batch_idx, (data_s, data_i, target) in enumerate(train_loader):
        data_s, data_i, target = Variable(data_s).cuda(), Variable(data_i).cuda(), Variable(torch.tensor(target)).cuda()
...
def sketch_image_encoder(sketch_path,image_path,sketch_data,image_data):
...
def main():
    ...
    sketch_image_encoder(args.sketch_path,args.img_path,args.sketch_temp_data,args.img_temp_data)
